...
EOF;
    ...
    public function ListGoodsWithSelected($GoodsID) {
      ...
      $res = parent::executeSQL($sql, nulll);
      $list = "<select name='GoodsID'>\n";
      $list .= "<option value='-99'>-- 選択してください --</option>";
      foreach ($rows=$res->fetchAll(PDO::FETCH_ASSOC) as $row) {
...
EOF;
      foreach ($rows = $res->fetchAll(PDO::FETCH_NUM) as $row) {
        ...
        $data .= <<< EOF
          ...
          <td><form action="" method="post">
            <input type="hidden" name="id" value="{$row[0]}">
            <input type="submit" name="deletedetail" value="削除" onClick="return CheckDelete()">
          </form></td>\n
EOF;
        $data .= "</tr>";
      }
    ...
    public function TotalAmount($SalesDate, $CustomerID) {
      //伝票の合計額
      $sql = <<< EOF
        SELECT sum(salesinfo.Quantity*goods.Price) FROM salesinfo INNER JOIN goods ON salesinfo.GoodsID = goods.GoodsID WHERE salesinfo.SalesDate = ? AND salesinfo.CustomerID = ?
EOF;
      $array = array($SalesDate, $CustomerID);
      $res = parent::executeSQL($sql, $array);
...
EOF;
      ...
      foreach($rows = $res->fetchAll(PDO::FETCH_NUM) as $row) {
        ...
        $data .= <<< EOF
          ...
          <td><form action="" method="post">
            <input type="hidden" name="SalesDate" value="{$row[0]}">
            <input type="hidden" na,e="CustomerID" value="{$row[1]}">
