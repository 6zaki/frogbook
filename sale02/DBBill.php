...
class DBBill extends DB{
  ...
  private function SelectCustomers($startDate, $endDate) {
    ...
    FROM salesinfo INNER JOIN customer ON salesinfo.CustomerID=customer.CustomerID WHERE salesinfo.SalesDate BETWEEN ? AND ?
...
EOF;
  ...
  public function totalAmount($saleDate, $endDate, $CustomerID) {
    //請求書の合計額
    $sql = <<< EOF
    SELECT sum(salesinfo.Quantity*goods.Price) FROM salesinfo INNER JOIN goods ON salesinfo.GoodsID = goods.GoodsID WHERE (salesinfo.SalesDate BETWEEN ? AND ?) AND salesinfo.CustomerID = ?
EOF;
